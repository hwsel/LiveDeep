...
class Solver(object):
    ...
    def load_model(self, NetworkType):
        if self.cuda:
        ...
        if NetworkType == "AlexNet":
            self.model = AlexNet().to(self.device)
        if NetworkType == "VGG11":
            self.model = VGG11().to(self.device)
        if NetworkType == "VGG13":
            self.model = VGG13().to(self.device)
        if NetworkType == "VGG16":
            self.model = VGG16().to(self.device)
        if NetworkType == "VGG19":
    ...
    def run(self,IndeV,IndeU,Tk,EpoMax,NetworkType):
        #self.load_data()
        self.load_model(NetworkType)
        '''
        accuracy = 0
        ...
        for FL in FileList:
            ...
            TpB = '_' + NetworkType + '_'
            CSVfilename = '00A' + TpB + FL + '_' + str(IndeU) + 'Tk_EpoMax' + str(Tk) + '_' + str(
                EpoMax) + '_AccAndBandC.csv'
            fileobj = open(CSVfilename, 'w', newline='')  
            CSVfilenameTime = '00A' + TpB + FL + '_' + str(IndeU) + 'Tk_EpoMax' + str(Tk) + '_' + str(
                EpoMax) + '_TimeConsumptionC.csv'
            ...
            TmpName = '0A' + TpB + FL + '_' + str(IndeU) + 'Tk_EpoMax' + str(Tk) + '_' + str(EpoMax) + '_B.avi'
            ...
            while (True):
                ...
                for i in range(bufLen):
                    ...
                    if cv.waitKey(1) & 0xFF == ord('q'):
                        break
                Out=data.(LocationPerFrame,countShow)
    ...
    def runB(self,IndeV,IndeU,Tk,EpoMax,NetworkType):
        #self.load_data()
        self.load_model(NetworkType)
        '''
        accuracy = 0
        ...
        TpB='_'+NetworkType+'_NOLifeLong_'
        CSVfilename='00A'+TpB+FL+'_'+str(IndeU)+'Tk_EpoMax'+str(Tk)+'_'+str(EpoMax)+'_AccAndBandC.csv'
        fileobj = open(CSVfilename, 'w', newline='')  
        CSVfilenameTime ='00A'+TpB+ FL +'_'+str(IndeU)+'Tk_EpoMax'+str(Tk)+'_'+str(EpoMax)+'_TimeConsumptionC.csv'
        fileobjT = open(CSVfilenameTime, 'w', newline='')  
...
if __name__ == '__main__':  # run python main.py --grid 5 --index 1
    ...
    #NetworkType="VGG13"
    '''
    NetworkTypeLS=["VGG11","VGG13","VGG16","VGG19","AlexNet"]
    for j in range(1, 2):#49
        #for i in range(14):#9
        for NetworkType in NetworkTypeLS:
            for TK in TKL:
                for Epomax in EpomaxL:
                    solver = Solver(args)
                    solver.run(i,j,TK,Epomax,NetworkType)
    '''
    #no lifelong
    NetworkTypeLS = ["VGG11", "VGG13", "VGG16", "VGG19", "AlexNet"]
    for j in range(1, 2):  # 49
        for i in range(14):#9
            for NetworkType in NetworkTypeLS:
                for TK in TKL:
                    for Epomax in EpomaxL:
                        solver = Solver(args)
                        solver.runB(i, j, TK, Epomax, NetworkType)
